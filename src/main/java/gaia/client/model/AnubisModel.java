...
public class AnubisModel extends EntityModel<AnubisRenderState> implements HeadedModel, ArmedModel {
	...
	@Override
	public void setupAnim(AnubisRenderState state) {
		...
		if (state.animationState == 1) {
			animationThrow();
		}

		if (state.animationState == 2) {
			animationCast();
		}

		// body
		tail.yRot = Mth.cos(((state.ageInTicks * 7) * Mth.DEG_TO_RAD)) * (5 * Mth.DEG_TO_RAD);
