...
public class SuccubusModel extends EntityModel<SuccubusRenderState> implements HeadedModel, ArmedModel {
	...
	@Override
	public void setupAnim(SuccubusRenderState state) {
		...
		// body
		rightwing.yRot = Mth.cos(state.ageInTicks * 0.3F + (float) Math.PI) * 1.0F * state.walkAnimationSpeed * 0.5F;
		leftwing.yRot = Mth.cos(state.ageInTicks * 0.3F) * 1.0F * state.walkAnimationSpeed * 0.5F;
		--rightwing.yRot;
		++leftwing.yRot;
