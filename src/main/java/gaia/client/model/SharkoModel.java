...
public class SharkoModel extends EntityModel<SharkoRenderState> implements HeadedModel, ArmedModel {
	...
	@Override
	public void setupAnim(SharkoRenderState state) {
		...
		if (state.animationState == 0) {
			...
			rightarm.zRot += (Mth.cos(state.ageInTicks * 0.09F) * 0.05F + 0.05F) + (Mth.DEG_TO_RAD * 15);
			rightarm.xRot += (Mth.sin(state.ageInTicks * 0.067F) * 0.05F) + (Mth.DEG_TO_RAD * 10);
			leftarm.zRot -= (Mth.cos(state.ageInTicks * 0.09F) * 0.05F + 0.05F) + (Mth.DEG_TO_RAD * 15);
			leftarm.xRot -= (Mth.sin(state.ageInTicks * 0.067F) * 0.05F) - (Mth.DEG_TO_RAD * 10);
		} else {
			animationBuff();
		}

		// body
		tail1.yRot = Mth.cos((Mth.DEG_TO_RAD * ((float) state.ageInTicks * 7))) * (2 * Mth.DEG_TO_RAD);
		tail2.yRot = Mth.cos((Mth.DEG_TO_RAD * ((float) state.ageInTicks * 7))) * (4 * Mth.DEG_TO_RAD);
		tail3.yRot = Mth.cos((Mth.DEG_TO_RAD * ((float) state.ageInTicks * 7))) * (6 * Mth.DEG_TO_RAD);
