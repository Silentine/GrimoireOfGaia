...
public class EntityGaiaSpawnerSphinx extends EntityMobProp {

	private boolean spawned;

	public EntityGaiaSpawnerSphinx(World worldIn) {
		super(worldIn);
		/**
		 * Copy size of original mob
		 * 
		 * @see EntityGaiaSphinx
		 */
		setSize(1.2F, 1.8F);
		spawned = false;
	}
	...
	private void setSpawn() {
		EntityGaiaSphinx entity;

		entity = new EntityGaiaSphinx(world);
	...
	public boolean getCanSpawnHere() {
		if (world.getDifficulty() == EnumDifficulty.PEACEFUL && GaiaConfig.SPAWN.spawnLevel3Rain) {
			return posY > ((!GaiaConfig.SPAWN.disableYRestriction) ? 60D : 0D) && world.isRaining() && world.getDifficulty() != EnumDifficulty.PEACEFUL && isValidLightLevel() && super.getCanSpawnHere();
		} else {
			return posY > ((!GaiaConfig.SPAWN.disableYRestriction) ? 60D : 0D) && world.getDifficulty() != EnumDifficulty.PEACEFUL && isValidLightLevel() && super.getCanSpawnHere();
		}
	}
	/* SPAWN CONDITIONS */
}
