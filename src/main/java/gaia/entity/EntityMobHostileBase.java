...
public abstract class EntityMobHostileBase extends EntityMob {
    ...
    /*
     * @SideOnly(Side.CLIENT) private void spawnParticles(EnumParticleTypes
     * particleType) { for (int i = 0; i < 5; ++i) { double d0 =
     * this.rand.nextGaussian() * 0.02D; double d1 = this.rand.nextGaussian() *
     * 0.02D; double d2 = this.rand.nextGaussian() * 0.02D;
     * this.world.spawnParticle(particleType, this.posX +
     * (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width,
     * this.posY + 1.0D + (double)(this.rand.nextFloat() * this.height),
     * this.posZ + (double)(this.rand.nextFloat() * this.width * 2.0F) -
     * (double)this.width, d0, d1, d2, new int[0]); } }
     */
    ...
    public void beaconDebuff(Potion effect, int duration, int power, int range) {
        if (!this.world.isRemote) {
             ...
             * this.world.<EntityPlayer>getEntitiesWithinAABB(EntityPlayer.
             * class, axisalignedbb); for (EntityPlayer entityplayer : list) {
             * entityplayer.addPotionEffect(new PotionEffect(effect, duration,
             * power, true, true)); }
             */
        }
    }
