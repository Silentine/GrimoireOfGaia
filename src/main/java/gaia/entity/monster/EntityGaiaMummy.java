...
public class EntityGaiaMummy extends EntityMobHostileBase {
	...
	public EntityGaiaMummy(World par1World) {
		super(par1World);
	...
	protected void applyEntityAttributes() {
		...
		this.getEntityAttribute(SharedMonsterAttributes.KNOCKBACK_RESISTANCE).setBaseValue(0.50D);
	}
	...
	public boolean attackEntityAsMob(Entity entityIn) {
		if (super.attackEntityAsMob(entityIn)) {
			if (entityIn instanceof EntityLivingBase) {
				...
				if (byte0 > 0) {
					((EntityLivingBase)entityIn).addPotionEffect(new PotionEffect(MobEffects.HUNGER, byte0 * 10, 0));
				}
			}
	...
	public void onLivingUpdate() {
		if (this.worldObj.isDaytime() && !this.worldObj.isRemote) {
			...
			if (f > 0.5F && this.rand.nextFloat() * 30.0F < (f - 0.4F) * 2.0F && this.worldObj.canSeeSky(this.getPosition())) {
				this.setFire(8);
				this.attackEntityFrom(DamageSource.outOfWorld, EntityAttributes.maxHealth1 * 0.10F);
			}
		}
