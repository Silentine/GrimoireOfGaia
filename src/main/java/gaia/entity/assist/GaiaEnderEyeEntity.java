...
public class GaiaEnderEyeEntity extends AbstractMobAssistEntity implements IEnderMob { //, IFlyingMob TODO: Flying
    ...
    private static final DataParameter<Boolean> SCREAMING = EntityDataManager.<Boolean>createKey(GaiaEnderEyeEntity.class, DataSerializers.BOOLEAN);

    private int targetChangeTime;

    public GaiaEnderEyeEntity(EntityType<? extends GaiaEnderEyeEntity> entityType, World world) {
    ...
    public void giveSpecificTasks() {
        ...
        this.targetSelector.addGoal(2, new GaiaEnderEyeEntity.FindPlayerGoal(this));
    }
    ...
    public void livingTick() {
        if (world.isRemote) {
            for (int i = 0; i < 2; ++i) {
                world.addParticle(ParticleTypes.PORTAL, getPosX() + (rand.nextDouble() - 0.5D) * getWidth(), this.getPosY() + rand.nextDouble() * getHeight() - 0.25D, this.getPosZ() + (rand.nextDouble() - 0.5D) * getWidth(), (rand.nextDouble() - 0.5D) * 2.0D, -rand.nextDouble(), (rand.nextDouble() - 0.5D) * 2.0D);
            }
        }
    ...
    static class FindPlayerGoal extends NearestAttackableTargetGoal<PlayerEntity> {
        private final GaiaEnderEyeEntity dragonGirl;
