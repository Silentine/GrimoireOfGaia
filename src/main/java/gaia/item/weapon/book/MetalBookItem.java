...
public class MetalBookItem extends WeaponBookItem {
	public MetalBookItem(ToolMaterial material, Properties properties) {
	...
	public void appendHoverText(ItemStack itemStack, TooltipContext context, TooltipDisplay display, Consumer<Component> builder, TooltipFlag tooltipFlag) {
		...
		builder.accept(Component.translatable(MobEffects.NAUSEA.value().getDescriptionId()).append(" II (0:04)"));
	}
	...
	public void executeHurtEffect(ItemStack stack, LivingEntity target, LivingEntity attacker) {
		attacker.level().playSound((Player) null, attacker.getX(), attacker.getY(), attacker.getZ(), GaiaSounds.BOOK_HIT.get(), SoundSource.NEUTRAL,
				1.0F, 1.0F);
		target.addEffect(new MobEffectInstance(MobEffects.NAUSEA, 80, 1));
	}
}
