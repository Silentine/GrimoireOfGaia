...
//	}
	...
	public void addInformation(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
		...
		if (shiftPressed) {
			...
			tooltip.add(new TranslationTextComponent("effect.minecraft.jump_boost"));
		} else {
	...
	public void doEffect(LivingEntity player, ItemStack item) {
		if (player.getActivePotionEffect(Effects.JUMP_BOOST) != null) {
			player.removePotionEffect(Effects.JUMP_BOOST);
		}

		player.addPotionEffect(new EffectInstance(Effects.JUMP_BOOST, 5 * 20, 1, true, false));
	}
